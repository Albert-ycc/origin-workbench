.PHONY: help makehelp dev server daemon cli multica build test migrate-up migrate-down sqlc seed clean setup start stop check e2e worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
selfhost: ## Create .env if needed, then build and start the Origin local backend
	@if [ ! -f .env ]; then \
	...
	@echo "==> Building and starting Origin backend from this checkout..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo ""; \
		echo "Image: origin-backend:dev"; \
		echo ""; \
		echo "Desktop login uses the local name + avatar flow."; \
		echo ""; \
		echo "Next — run the desktop shell:"; \
...
selfhost-build: ## Alias for selfhost
	@if [ ! -f .env ]; then \
		echo "==> Creating .env from .env.example..."; \
		cp .env.example .env; \
		JWT=$$(openssl rand -hex 32); \
...
start: ## Start the backend for the current checkout (Origin uses desktop only — run desktop with `pnpm dev:desktop`)
	$(REQUIRE_ENV)
	@echo "Using env file: $(ENV_FILE)"
...
stop: ## Stop backend process for the current checkout
	$(REQUIRE_ENV)
...
check: ## Run typecheck, TS tests, Go tests, and desktop build for the current checkout
	$(REQUIRE_ENV)
	@ENV_FILE="$(ENV_FILE)" bash scripts/check.sh

e2e: ## Run Playwright origin smoke with a local backend for the current checkout
	$(REQUIRE_ENV)
	@ENV_FILE="$(ENV_FILE)" bash scripts/e2e-origin-smoke.sh
