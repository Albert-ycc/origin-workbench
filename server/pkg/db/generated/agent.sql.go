...
const getWorkspaceAgentActivity365d = `-- name: GetWorkspaceAgentActivity365d :many
SELECT
    atq.agent_id,
...
type GetWorkspaceAgentActivity365dRow struct {
	AgentID     pgtype.UUID        `json:"agent_id"`
...
func (q *Queries) GetWorkspaceAgentActivity365d(ctx context.Context, workspaceID pgtype.UUID) ([]GetWorkspaceAgentActivity365dRow, error) {
	rows, err := q.db.Query(ctx, getWorkspaceAgentActivity365d, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetWorkspaceAgentActivity365dRow{}
	for rows.Next() {
		var i GetWorkspaceAgentActivity365dRow
		if err := rows.Scan(
			&i.AgentID,
			&i.Bucket,
			&i.TaskCount,
