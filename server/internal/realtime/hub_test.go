...
func TestDefaultAllowedOriginsIncludePackagedDesktopFileOrigin(t *testing.T) {
	...
	if !found {
		t.Fatalf("default websocket origins = %v, want file://", origins)
	}

	previous := allowedWSOrigins.Load().([]string)
	SetAllowedOrigins(origins)
	t.Cleanup(func() {
		SetAllowedOrigins(previous)
	})

	req := httptest.NewRequest(http.MethodGet, "/ws", nil)
	req.Header.Set("Origin", "file://")
	if !checkOrigin(req) {
		t.Fatal("checkOrigin rejected packaged desktop file:// origin")
	}
}
