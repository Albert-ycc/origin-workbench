...
// GetWorkspaceAgentActivity365d returns per-agent daily task counts for the
// last 365 days, anchored on completed_at. Single workspace-wide read backs
// both the Agents list sparkline (uses the trailing 7 buckets) and the
// agent detail year heatmap — one fetch is cheaper than two. Front-end fills
// missing days with zero; the back-end omits
// empty buckets to keep the response small.
func (h *Handler) GetWorkspaceAgentActivity365d(w http.ResponseWriter, r *http.Request) {
	workspaceID := h.resolveWorkspaceID(r)
	if _, ok := h.workspaceMember(w, r, workspaceID); !ok {
		return
	}

	rows, err := h.Queries.GetWorkspaceAgentActivity365d(r.Context(), parseUUID(workspaceID))
	if err != nil {
